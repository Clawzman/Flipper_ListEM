...
static const uint8_t STRUCT_TAILS[] = {
...
prefix = usable[comb_prefix_index];
    ...
    prefix = usable[
        s->per_prefix ? (prefix_index % n) : (rand() % n)
    ];
}
...
if(s->submenu == SubmenuSequential) {
    ...
    canvas_draw_str(c,10,38,
        s->per_prefix ? "Per-prefix: ON" : "Per-prefix: OFF"
    );

    snprintf(
        l,sizeof(l),
        "Direction: %s",
        s->seq_reverse ? "Reverse" : "Forward"
    );
...
if(s->submenu == SubmenuFuzz) {
    ...
    if(e->key==InputKeyUp && s->fuzz_focus > FuzzBoundary)
        ...
        if(s->fuzz_focus==FuzzBoundary)
            s->fuzz_boundary ^= 1;
        else if(s->fuzz_focus==FuzzBitflip)
            s->fuzz_bitflip ^= 1;
...
int32_t rfid_fuzzer_app(void* p) {
    ...
    AppState s = {
        ...
        .mode=GenModeSequential,.structured_type = StructSeq,.focus=FocusProtocol,
